...
var Offering = keystone.list('Offering');
...
exports = module.exports = function(req, res) {
	...
	// Init locals
		locals.section = 'offering';
		locals.data = [];

	// Load the current Offering
	view.on('init', function(next) {
		var q = Offering.model.find();
		q.exec(function(err, results) {
			locals.data.offerings = results;
			next(err);
		});
	});
