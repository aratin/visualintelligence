...
var SoftwareProduct = keystone.list('SoftwareProduct');
...
exports = module.exports = function(req, res) {
	...
	// Init locals
		locals.section = 'softwareproduct';
		locals.data = [];

	// Load the current SoftwareProduct
	view.on('init', function(next) {
		var q = SoftwareProduct.model.find();
		q.exec(function(err, results) {
			locals.data.softwareproducts = results;
			next(err);
		});
	});
